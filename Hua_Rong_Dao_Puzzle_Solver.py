    ...
    def possible_move_finder(self):
        ...
        for elem in empty_spaces:
            ...
            #Viability rules for solo space:
                #1. Singles always allowed
                #2. If 2x1: ^ allowed if below, ^ allowed if on top, > allowed if on left, < allowed if on right
                #3. If 2x2: Never allowed
            if x_coord > 0:
            ...
            if y_coord > 0:
                if self.board.grid[y_coord-1][x_coord] in single_allowable_above:
                    if self.board.grid[y_coord - 1][x_coord] == 'v':
...
def dfs(current_state, output_filename):
    """
    Finds the solution using depth first search and creates a file containing all the moves made.

    :param current_state: Initial state of the game.
    :type current_state: State class
    :param output_filename: File to document moves made.
    ...
    while len(frontier) > 0:
        cur = frontier.pop()
        if str(cur.board.grid) not in explored:
            explored.add(str(cur.board.grid))
...
def a_star(current_state, output_filename):
    """
    Finds the solution using A* Algorithm with Manhattan distance heuristic and creates a file containing all the moves made.
    ...
    while len(pq) > 0:
        ...
        if str(cur.board.grid) not in explored:
            ...
            for move in possible_moves:
                    ...
                    heappush(pq, (new_state.f, new_state.id, new_state))
    return None
